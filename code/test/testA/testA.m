...
%     end
    ...
    [centroids, bboxes, mask, mask0] = detectObjects(frame);
    %subplot(311); imshow(mask);
    %subplot(312); imshow(frame);
    
    
    testMap0(:,:,1)=single(frame(:,:,1).*mask0);
    testMap0(:,:,2)=single(frame(:,:,2).*mask0);
    testMap0(:,:,3)=single(frame(:,:,3).*mask0);
    imshow(testMap0);
    testMap(:,:,1)=single(frame(:,:,1).*mask);
    testMap(:,:,2)=single(frame(:,:,2).*mask);
    testMap(:,:,3)=single(frame(:,:,3).*mask);
    imshow(testMap);
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects(fileName,ext)
        % Initialize Video I/OdetectObjects
        ...
        % the background. It outputs a binary detectObjectsmask, where the pixel value
        % of 1 corresponds to the foreground and the value of 0 corresponds
        % to the background. 
        
        obj.detector = vision.ForegroundDetector('NumGaussians', 3, ...
            'NumTrainingFrames', 40, 'MinimumBackgroundRatio', 0.3);
...
% The state consists of information used for detectObjectsdetection to track assignment,
...
% remove noisy pixels and to fill the holes in the remaining blobs.  
    ...
    function [centroids, bboxes, mask, mask0] = detectObjects(frame)
