function [playerCollection ] = updateStats( playerCollection )
    ...
    for i=1:count
        ...
        %size of trackX remains constant
        %but size of trackX_net is steps
        %similarly smoothtrackX_net
        onePlayer.trackX_net=NaN(steps,1);
        onePlayer.trackY_net=NaN(steps,1);
        onePlayer.smoothTrackY_net=NaN(steps,1);
        onePlayer.smoothTrackX_net=NaN(steps,1);
